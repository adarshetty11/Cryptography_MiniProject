...
class RC2():
    ...
    def __init__(self, key,key_bit):
        ...
        key_byte = (key_bit+7)//8
        key_mask = 255 % 2^(8 + key_bit - 8*key_byte)
        ...
        rc2_key[128 - len(key)] = sbox[rc2_key[128 - len(key)] & key_mask]
    ...
    def encrypt(self, input_buffer, IV, padding):
    ...
    def decrypt(self, input_buffer, IV, padding):
        ...
        for block_counter in range(no_of_blocks):
            ...
            if block_counter == 0:
                for i in range(RC2_BLOCK_SIZE):
                    crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = block_result[i] ^ ord(IV[i])
            else:
                for i in range(RC2_BLOCK_SIZE):
...
if __name__=='__main__':
    ...
    key_limit = int(input("Enter effective key limit(in bits) : "))
    iv = "\x01\x02\x03\x04\x05\x06\x07\x08"

    #encryption
    rc2 = RC2(key,key_limit)
    encrypted = rc2.encrypt(plaintext,iv,PADDING_PKCS5)
    print("\nEncrypted msg :\n",encrypted.hex())
 
    #decryption.
    rc2 = RC2(key,key_limit)
    decrypted = rc2.decrypt(encrypted,iv,PADDING_PKCS5)
